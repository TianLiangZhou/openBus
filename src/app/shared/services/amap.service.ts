...
import {
  ...
  LineDetailResponse,
  LineResponse,
  LineStationResponse, NearByLinesResponse, PoiRealBus, PoiSearchResponse
} from "../data/amap";

@Injectable({
  providedIn: 'root'
})
export class AmapService implements PoiRealBus {
  ...
  getIpLocation(ip?: string) {
    let url = this.gateway + '/v3/ip?key=' + this.secret;
